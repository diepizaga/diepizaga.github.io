...
| EP-8 | **Alto** | `profiles` tenía 0 filas porque `loadProfile()` intentaba crear la fila con columnas (`display_name`, `email`) que **no existen** en la tabla real (solo existen `id`, `username`, `created_at`) — roto desde el primer commit del feature (06-jun-2026), no una regresión de esta sesión. Salió a la luz al validar Bloque B (la FK `groups_created_by_fkey` lo expuso). | **Resuelto (30-jul-2026)** — Bloque K finalizado: modelo de identidad definido, código adaptado a `username` (sin cambio de esquema), validado con datos reales en Safari. Ver DESIGN_MAP.md § Bloque K. **Nota de transparencia:** la auditoría original de columnas de `profiles` (hecha durante el diseño de Bloque B) afirmaba erróneamente que `display_name`/`email` existían — era una lectura del código, no de la base real. Corregido en DESIGN_MAP.md § Bloque K, "Corrección de auditoría". |
| EP-9 | **Alto** | `createGroup()` usaba `Prefer: return=representation`, cuya lectura de vuelta pasaba por `groups_read` (exigía membresía) — Postgres revertía el `INSERT` completo. `joinGroup()` tenía el mismo problema de fondo al buscar por invite_code. | **Resuelto (30-jul-2026)** — Bloque L finalizado: `groups_read` acepta también al creador, y se agregó `resolve_invitation()` (función de solo lectura, mínima exposición) para que `joinGroup()` encuentre el grupo sin ampliar la superficie de lectura hacia otros usuarios. Ver DESIGN_MAP.md § Bloque L. |
| EP-10 | **Medio** | `groups` no tiene ninguna política de **DELETE** — ni el creador puede borrar un grupo vía la API normal. | **Pendiente** — hallazgo independiente, sin alcance ni bloque asignado. No bloquea nada (no hay feature de borrar grupo en la UI hoy). |
| EP-11 | **Medio** | `group_members.role` no existe — las únicas columnas reales son `group_id` y `user_id`. El código asume `role` en 3 lugares (crear, unirse, badge "admin"). Auditado: el producto no necesita ese dato — `groups.created_by` ya identifica al creador; el único consumo real (el badge) es derivable de ahí sin duplicar nada. | **Auditado (30-jul-2026)** — pendiente de diseño/alcance (decidir entre sacar `role` del código o agregar la columna). Ver DESIGN_MAP.md § Bloque L, "Hallazgo relacionado". |
...
|---|---|---|---|
| SEC-1 | **Crítico** | `profiles`, `groups`, `group_members` sin RLS habilitado — accesibles por cualquiera con el `anon key` (público, en el HTML). Sin precondiciones. | **Resuelto (30-jul-2026)** — RLS habilitado, `profiles_read` corregida, validado con cuenta real. Ver DESIGN_MAP.md § Bloque B. SECURITY_AUDIT_RLS.md § Riesgo 1 |
...
| SEC-3 | **Alto** | Cadena `group_members` (sin RLS) → `watchlist.group_read`: permite forjar membresía en cualquier grupo y leer los watchlists compartidos de sus miembros reales. Requiere que la función de grupos ya tenga datos reales (hoy aparenta no ser el caso). | **Parcialmente mitigado (30-jul-2026)** — `group_members` ya tiene RLS (Bloque B), `gm_insert` exige `user_id = auth.uid()` (no podés sumar a otro). Sigue sin validar el código de invitación — cualquiera puede unirse a cualquier `group_id` conocido. Cierre completo es Bloque D (prioridad baja, diferido). SECURITY_AUDIT_RLS.md § Riesgo 3 |
