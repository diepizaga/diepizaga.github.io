...
| EP-9 | **Alto** | `createGroup()` usa `Prefer: return=representation`, cuya lectura de vuelta pasa por `groups_read` (exige membresía) — como todavía no sos miembro del grupo recién creado, Postgres revierte el `INSERT` completo. Crear un grupo falla siempre, para cualquier cuenta. Además, `groups` no tiene ninguna política de **DELETE** — ni el creador puede borrar un grupo vía la API normal. | **Auditado (30-jul-2026)** — bloque independiente **Bloque L** abierto, diseño pendiente. Ver DESIGN_MAP.md § Bloque L. |
