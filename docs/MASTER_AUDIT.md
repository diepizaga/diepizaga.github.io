...
| EP-8 | **Alto** | `profiles` tiene 0 filas porque `loadProfile()` intenta crear la fila con columnas (`display_name`, `email`) que **no existen** en la tabla real (solo existen `id`, `username`, `created_at`) — roto desde el primer commit del feature (06-jun-2026), no una regresión de esta sesión. Salió a la luz al validar Bloque B (la FK `groups_created_by_fkey` lo expuso). | **Auditado (30-jul-2026)** — bloque independiente **Bloque K** abierto, diseño pendiente. Ver DESIGN_MAP.md § Bloque K. **Nota de transparencia:** la auditoría original de columnas de `profiles` (hecha durante el diseño de Bloque B) afirmaba erróneamente que `display_name`/`email` existían — era una lectura del código, no de la base real. Corregido en DESIGN_MAP.md § Bloque K, "Corrección de auditoría". |
